...
def smoother_l1(Ag, bg, Ng, igg, ksptype, pctype):
    ...
    petsc_options['smoother1_ksp_chebyshev_eigenvalues'] = '0.10393855,\
                                                           2.078771'
    ksp.setTolerances(max_it=Ng)
    ksp.setOperators(Ag)
    ksp.setFromOptions()
...
def smoother_l2(Ag, bg, Ng, igg, ksptype, pctype):
    ...
    petsc_options['smoother2_ksp_chebyshev_eigenvalues'] = '0.10563305,\
                                                            2.112661'
    ksp.setTolerances(max_it=Ng)
    ksp.setOperators(Ag)
    ksp.setFromOptions()
...
def smoother_l3(Ag, bg, Ng, igg, ksptype, pctype):
    ...
    petsc_options['smoother3_ksp_chebyshev_eigenvalues'] = '0.11878675, \
                                                            2.375735'
    ksp.setTolerances(max_it=Ng)
    ksp.setOperators(Ag)
    ksp.setFromOptions()
...
def mg(Ahlist, bh, uh, prolongation, restriction, N_cycles, N_levels,
    ...
    for num_cycle in range(N_cycles):
        ...
        for i in range(N_levels - 1):
            ...
            smoother_cor(Ahlist[i], blist[i], nu, uhlist[i], i,
                         ksptype, pctype, Acorlist[i], poorlist[i])
        ...
        for j in range(N_levels - 2, -1, -1):
            ...
            smoother_cor(Ahlist[j], blist[j], nu, uhlist[j], j,
                         ksptype, pctype, Acorlist[j], poorlist[j])
...
# matrix_split.matrix_split(Alist[0], bad_level1)
# matrix_split.matrix_split(Alist[1], bad_level2)
# matrix_split.matrix_split(Alist[2], bad_level3)
# emax_list = [2.078771, 2.112661, 2.375735]
# emin_list = [i/20 for i in emax_list]
# quit()
